...
//				rayHandler.x2, rayHandler.y2);
    ...
    protected void updateDynamicShadowMeshes() {
        ...
        for (b2ShapeId fixture : affectedFixtures) {
            ...
            b2Vec2 b2Vec2 = Box2d.b2Body_GetWorldCenterOfMass(body);
            center.set(b2Vec2.x(), b2Vec2.y());
            ...
            if (type == b2ShapeType.b2_polygonShape) {
                ...
                for (int n = 0; n < vertexCount; n++) {
                    b2Vec2 vec = pShape.vertices().get(n);

                    Box2d.b2Body_GetWorldPoint(this.body,vec);
                    tmpVerts.add(new Vector2(vec.x(), vec.y()));

                    tmpEnd.set(tmpVec).sub(lstart).limit2(0.0001f).add(tmpVec);
                    if (Box2dPlus.b2ShapeTestPoint(fixture,tmpEnd)) {
                        if (minN == -1) {
                            minN = n;
                        }
                        maxN = n;
    ...
    public void setIgnoreBody(b2BodyId body) {
        this.body = body;
